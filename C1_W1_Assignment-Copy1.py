...
type(freqs)
out = dict(itertools.islice(freqs.items(), 5)) 
out
...
def gradientDescent(x, y, theta, alpha, num_iters):
    ...
    for i in range(0, num_iters):
        ...
        J = (np.matmul((y.T),(np.log(h))) + np.matmul((1-y.T),(np.log(1-h)))) *(-1/m)
...
#z = np.dot(tmp_X,np.zeros((3, 1)))
#z
#h = sigmoid(z)
#h
#tmp_Y = (np.random.rand(10, 1) > 0.35).astype(float)
#print(tmp_Y)
#J = np.matmul((tmp_Y.T),(np.log(h)))
#J
...
print("The cost after training is .", tmp_J)
...
def test_logistic_regression(test_x, test_y, freqs, theta):
    ...
    y_hat = np.reshape(np.array(y_hat), (len(y_hat),1)) 

    accuracy = float(sum((y_hat==test_y))/len(test_x))
...
print("Logistic regression model's accuracy = ", tmp_accuracy)
