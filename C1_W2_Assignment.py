...
def train_naive_bayes(freqs, train_x, train_y):
    ...
    for word in vocab:
        ...
        if pair_pos in freqs:
            freq_pos = freqs[pair_pos] 
        else:
            freq_pos=0
            #print(freq_pos)
        pair_neg = (word, 0)
        if pair_neg in freqs:
            freq_neg = freqs[pair_neg]
        else:
            freq_neg=0
                
            #print(freq_neg)
        #freq_neg = [pair[1] for pair in freqs if pair[0][1]==0]
...
D_pos = np.sum(train_y==0)
#D_pos

x=[pair[1] for pair in freqs]
x[-10:-5]
...
#freq_pos = [pair[1] for pair in freqs if pair[0][1]==1]
...
def get_words_by_threshold(freqs, label, threshold):
    ...
    for key in freqs.keys():
        ...
        A = get_ratio(freqs, word)
        pos_neg_ratio = A['ratio']

        # if the label is 1 and the ratio is greater than or equal to the threshold...
        if label == 1 and pos_neg_ratio>=threshold:

            word_list[word] = A; # Add the pos_neg_ratio to the dictionary
